...
def index_rows():
    ...
    for p0 in rows:
        ...
        out.append({
            ...
            "status": p.get("current_status") or status_of(v), "owner_rep": p.get("owner_rep") or "",
            "reps": h["reps_involved"], "visits": len(v), "contacts": h.get("contacts", []),
            "last": last_visit,
            ...
            "last_outcome": p.get("last_outcome") or "",
        })
    return out
...
def property_detail(pid: int):
    ...
    return {"property": dict(p), "visits": h["visits"], "contacts": h.get("contacts", []),
...
def team():
    ...
    return {"overlaps": tools.find_cross_rep_gaps(con()),
            ...
            "no_requirement": no_req, "opportunities": opportunities}
...
def add_visit(v: VisitIn):
    return tools.log_visit(con(), v.property_id, v.rep, v.outcome_category, v.remarks,
                           ...
                           logged_lon=v.logged_lon, contact_name=v.contact_name, contact_email=v.contact_email,
                           contact_phone=v.contact_phone, contact_designation=v.contact_designation,
    ...
    pid = con().insert(
        """INSERT INTO properties
           (property_name, brand, branch, parent_site, area, territory,
            ...
            location_confidence, exact_location, created_by)
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (p.name.strip(), p.brand.strip(), p.branch.strip(), "", p.area.strip(),
         "field_added", p.property_type, p.latitude, p.longitude,
         "rep_pin", "confirmed", p.exact_location.strip(), p.rep), pk="property_id")
    con().commit()
...
def commit_field_entry(v: FieldEntryIn):
    ...
    if v.new_property:
        ...
        result = tools.create_property_and_visit(
            ...
            {"name": v.property_name, "area": v.area, "property_type": v.property_type,
             ...
             "longitude": v.longitude, "exact_location": v.exact_location, "rep": v.rep},
            v.model_dump(),
        )
        return result
    ...
    return tools.log_visit(con(), v.property_id, v.rep, v.outcome_category, v.remarks,
                           ...
                           logged_lon=v.longitude, contact_name=v.contact_name, contact_email=v.contact_email,
                           contact_phone=v.contact_phone, contact_designation=v.contact_designation,
                           exact_location=v.exact_location)
...
def route(origin_lat: float, origin_lon: float, points: str, mode: str = "WALK"):
    if not _valid_coord(origin_lat, origin_lon):
        raise HTTPException(400, "Invalid route origin coordinates")
    ...
    stops = [p for p in stops if _valid_coord(p.get("lat"), p.get("lon"))][:25]
    if not stops:
