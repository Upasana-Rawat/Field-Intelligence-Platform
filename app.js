...
async function boot(){
  ...
  }catch(e){$("#screen-home").innerHTML=`<div class="screen-inner"><div class="alert bad">Unable to load the application: ${esc(e.message)}</div></div>`}
}
...
function drawInternal(){
  ...
  S.props.forEach(p=>{
    if(p.lat==null||!shouldShowInternal(p))return;
...
function drawExternal(){
  ...
  S.external.forEach(p=>{if(p.lat==null)return;const m=L.circleMarker([p.lat,p.lon],{radius:5,color:"#2563eb",fillColor:"#dbeafe",fillOpacity:.9,weight:2}).bindTooltip(`${esc(p.name)} · Not in team database`);m.on("click",()=>openExternal(p));S.extLayer.addLayer(m)});
}
...
function drawPlanMap(){
  ...
  S.plan.forEach((p,i)=>{if(p.lat!=null){pts.push([p.lat,p.lon]);L.marker([p.lat,p.lon]).bindTooltip(`${i+1}. ${esc(p.name)}`).addTo(S.planLayer)}});
  if(S.me)L.marker([S.me.lat,S.me.lon],{icon:L.divIcon({className:"",html:'<div class="me-marker"></div>',iconSize:[18,18],iconAnchor:[9,9]})}).addTo(S.planLayer);
...
function renderNearList(){
  const el=$("#nearList");if(!el)return;const internal=(S.internalNearby||S.props.filter(p=>p.lat&&S.me).sort((a,b)=>a.distance_m-b.distance_m)).filter(shouldShowInternal);const ext=S.filters.unvisited?S.external:[];
