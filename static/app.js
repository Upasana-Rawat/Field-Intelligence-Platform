(() => {
"use strict";
const state={screen:"home",data:null,rep:localStorage.getItem("fi_rep")||"Rep A",coords:null,map:null,markers:[],nearby:[],filter:"all",recording:null,mediaRecorder:null,recordChunks:[],recordStarted:0};
...
function show(screen){state.screen=screen;document.querySelectorAll(".screen").forEach(s=>s.classList.toggle("active",s.dataset.screen===screen));document.querySelectorAll(".nav").forEach(n=>n.classList.toggle("active",n.dataset.go===screen));$("pageTitle").textContent={home:"Good day",nearby:"Near Me",log:"Log a visit",ask:"Field copilot",team:"Team intelligence",property:"Property"}[screen]||"Field Intelligence";if(screen==="nearby")setTimeout(()=>{initMap();state.map?.invalidateSize();loadNearby()},60);if(screen==="team")renderTeam();if(screen==="home")renderHome()}
...
function renderTeam(){
  ...
  const noReq=properties().filter(p=>String(p.status||p.current_status||"").toLowerCase()==="no requirement").length;
  $("teamStats").innerHTML=[["Opportunities",s.live??0],["No requirement",noReq],["Overlap alerts",s.overlaps??0],["Visits",s.visits??0]].map(x=>`<div class="team-card"><strong>${x[1]}</strong><span>${x[0]}</span></div>`).join("");
  $("overlapList").innerHTML=overlaps().length?overlaps().slice(0,12).map(o=>`<div class="list-card clickable" data-property="${esc(o.property_id||"")}"><div><div class="list-title">${esc(o.property_name)}</div><div class="list-sub">${esc((o.reps||[]).join(" · "))}</div><div class="list-meta"><span class="tag overlap">Overlap</span></div></div><b>→</b></div>`).join(""):`<div class="list-card"><div><div class="list-title">No overlap alerts</div><div class="list-sub">When multiple reps visit a property, it appears here.</div></div></div>`;
  const activity=state.data?.activity||[];
  $("activityList").innerHTML=activity.slice(0,12).map(v=>`<div class="list-card clickable" data-property="${esc(v.property_id)}"><div><div class="list-title">${esc(v.property_name)} · ${esc(v.rep)}</div><div class="list-sub">${esc(v.visit_date||v.created_at||"")} · ${esc(v.outcome_category||"")}</div><div class="list-sub">${v.contact_name?`Contact: ${esc(v.contact_name)}${v.contact_phone?` · ${esc(v.contact_phone)}`:""}${v.contact_email?` · ${esc(v.contact_email)}`:""}`:"No contact captured"}</div><div class="list-sub">${esc(v.remarks||"")}</div></div><b>→</b></div>`).join("")||`<div class="list-card"><div><div class="list-title">No field activity yet</div><div class="list-sub">Saved visits will appear here immediately.</div></div></div>`;
}

async function openGoogleSheet(){
  const url=state.data?.google_sheet_url||"";
  if(!url){
    $("sheetMessage").textContent="Google Sheet link is not configured in Render. Add GOOGLE_SHEET_URL to Environment Variables.";
    $("sheetMessage").hidden=false;
    toast("Google Sheet link is not configured.");
    return;
  }
  window.open(url,"_blank","noopener,noreferrer");
}
$("sheetBtn")?.addEventListener("click",openGoogleSheet);
...
function renderAll(){renderHome();renderTeam();if(state.screen==="nearby")renderMapAndList()}
bootstrap();
})();
