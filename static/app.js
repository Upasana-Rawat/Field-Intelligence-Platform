...
/* ------------------------------ Near Me */
function renderNear(){
  if(S.map){try{S.map.remove()}catch{} S.map=null; S.cluster=null; S.extLayer=null; S.meMarker=null; S.meCircle=null;}
...
function renderProperty(id){
  const d=S.propertyDetail;if(!d||!d.property)return;const p=d.property;const visits=d.visits||[];const last=visits[visits.length-1];const contacts=d.contacts||[];
...
function addToPlan(p){if(!p)return;const x={id:p.id||p.property_id,name:p.name||p.property_name,lat:p.lat??p.latitude,lon:p.lon??p.longitude,status:p.status,type:p.type||p.property_type};if(!validCoord(x.lat,x.lon)){toast("This property has no valid coordinates");return}if(!S.plan.some(q=>q.id&&q.id===x.id))S.plan.push(x);toast("Added to today’s plan");if(S.screen==="plan")renderPlan()}
...
/* ------------------------------ Plan */
function renderPlan(){
  if(S.planMap){try{S.planMap.remove()}catch{} S.planMap=null; S.planLayer=null; S.routeLayer=null;}
...
function renderDraft(){
 ...
 const matchLabel=d.newProperty?"New property — will be created":`Existing property — ${d.match.property_name}`;
 const c=d.match?.contacts?.[0]||{};
 $("#draftArea").innerHTML=`<div class="confirm-box">
 <div class="alert ${d.newProperty?"info":"good"}"><b>${esc(matchLabel)}</b>${d.candidates?.length?`<div class="meta">Top match confidence: ${Math.round((d.match?.match_score||0)*100)}%</div>`:""}</div>
 <div class="draft-grid"><div class="draft-item"><span>Property</span><b>${esc(d.property_name||"—")}</b></div><div class="draft-item"><span>Outcome</span><b>${esc(d.outcome_category||"—")}</b></div><div class="draft-item"><span>Requirement</span><b>${esc(d.requirement_detail||"—")}</b></div><div class="draft-item"><span>Follow-up</span><b>${esc(d.follow_up_state||"none")}</b></div><div class="draft-item"><span>Competitor</span><b>${esc(d.competitor_vendor||"None")}</b></div><div class="draft-item"><span>Remarks</span><b>${esc(d.remarks||"—")}</b></div></div>
 <div class="section-title" style="margin-top:14px">Contact & location intelligence</div>
 <div class="form-grid" style="margin-top:8px">
   <div class="field"><label>Contact person</label><input id="draftContactName" value="${esc(d.contact_name||c.name||"")}" placeholder="Name"></div>
   <div class="field"><label>Designation</label><input id="draftContactDesignation" value="${esc(d.contact_designation||c.designation||d.contact_role||"")}" placeholder="Facility Manager"></div>
   <div class="field"><label>Email</label><input id="draftContactEmail" type="email" value="${esc(d.contact_email||c.email||"")}" placeholder="name@example.com"></div>
   <div class="field"><label>Phone</label><input id="draftContactPhone" value="${esc(d.contact_phone||c.phone||"")}" placeholder="Phone number"></div>
   <div class="field" style="grid-column:1/-1"><label>Exact location</label><input id="draftExactLocation" value="${esc(d.exact_location||d.match?.exact_location||"")}" placeholder="Tower / building / floor / office"></div>
 </div>
 ${d.newProperty?`<div class="form-grid" style="margin-top:12px"><div class="field"><label>Area</label><input id="draftAreaInput" value=""></div><div class="field"><label>Property type</label><select id="draftType"><option value="tech_park">Tech Park</option><option value="coworking">Coworking</option><option value="business_park">Business Park</option><option value="corporate_office">Corporate Office</option><option value="commercial">Commercial</option></select></div><div class="field"><label>Latitude</label><input id="draftLat" value="${d.lat??""}"></div><div class="field"><label>Longitude</label><input id="draftLon" value="${d.lon??""}></div></div>`:""}
 <div class="actions" style="margin-top:14px"><button class="btn blue" id="confirmDraft">✓ Confirm & Save</button><button class="btn secondary" id="editDraft">Edit note</button></div></div>`;
 $("#confirmDraft").onclick=confirmDraft;$("#editDraft").onclick=()=>{$("#draftInput")?.focus();};
}
async function confirmDraft(){
 const d=S.draft;if(!d)return;
 try{
  const body={rep:S.rep,property_id:d.newProperty?null:d.match.id,new_property:d.newProperty,property_name:d.property_name,
   area:d.newProperty?$("#draftAreaInput")?.value||"":d.match?.area||"",property_type:d.newProperty?$("#draftType")?.value||"commercial":d.match?.type||"commercial",
   latitude:d.newProperty?Number($("#draftLat")?.value):S.me?.lat??d.match?.lat??null,longitude:d.newProperty?Number($("#draftLon")?.value):S.me?.lon??d.match?.lon??null,
   outcome_category:d.outcome_category,remarks:d.remarks||"",visit_date:d.visit_date||today(),requirement_detail:d.requirement_detail||"",competitor_vendor:d.competitor_vendor||"",pricing_note:d.pricing_note||"",contact_role:d.contact_role||"",follow_up_state:d.follow_up_state||"none",access_blocked:!!d.access_blocked,
   contact_name:$("#draftContactName")?.value?.trim()||d.contact_name||"",contact_designation:$("#draftContactDesignation")?.value?.trim()||d.contact_designation||d.contact_role||"",contact_email:$("#draftContactEmail")?.value?.trim()||d.contact_email||"",contact_phone:$("#draftContactPhone")?.value?.trim()||d.contact_phone||"",exact_location:$("#draftExactLocation")?.value?.trim()||d.exact_location||""};
  if(!validCoord(body.latitude,body.longitude)){delete body.latitude;delete body.longitude;}
  const result=await api("/api/field-entry",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});
  toast("Saved to the shared team database");S.draft=null;S.prefillProperty=null;await reloadData();openProperty(result.property_id||body.property_id)
